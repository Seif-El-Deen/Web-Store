...
cat_parent_id int,
CONSTRAINT fk_categories_parent foreign key (cat_parent_id) references category(cat_id)
);
...
constraint fk_prod_cat_prod_id foreign key (prod_cat_prod_id) references product(prod_id),
constraint fk_prod_cat_cat_id foreign key (prod_cat_cat_id) references category(cat_id)
);
...
constraint fk_orders_customer_id foreign key (ord_cust_id) references customer(cust_id)
);
...
constraint fk_ord_items_prod_id foreign key (ord_item_prod_id) references product(prod_id),
...
create table sale_history(
    ...
    constraint fk_sale_history_cust_id foreign key(customer_id) references customer(cust_id),
    constraint fk_sale_history_prod_id foreign key(product_id) references product(prod_id)
);